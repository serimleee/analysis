...
INNER JOIN 
    ...
    WHERE CAR_TYPE = '세단') B ON A.CAR_ID = B.CAR_ID
WHERE EXTRACT(MONTH FROM A.START_DATE) = 10
ORDER BY 1 DESC
...
FROM 
    (SELECT A.ANIMAL_ID, A.NAME, DATEDIFF(A.DATETIME,B.DATETIME) DAYS
     ,RANK()OVER(ORDER BY DATEDIFF(A.DATETIME,B.DATETIME) DESC) RANKK -- 이후,이전
    FROM ANIMAL_OUTS A
...
INNER JOIN 
    ...
    GROUP BY 1 
    HAVING COUNT(BOARD_ID) >= 3) C ON B.USER_ID = C.WRITER_ID
ORDER BY 1 DESC
